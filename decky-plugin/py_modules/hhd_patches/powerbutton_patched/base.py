...
def run_steam_shortpress():
    # OXP Apex patch: always hibernate on short press.
    # S0i3 suspend is broken on Strix Halo (kernel <6.18), so we bypass
    # supports_sleep() and force hibernate instead of steam://shortpowerpress.
...
def power_button_isa(cfg: PowerButtonConfig, should_exit: Event, emit):
    ...
    try:
        while not should_exit.is_set():
            ...
            if issue_systemctl:
                logger.error(
                    "Power button action did not work. Calling `systemctl hibernate`"
                )
